...
After installation, run the following command inside the container:

```bash
./benchmark/kvstore/kvstore_benchmark.sh
```

The default configuration uses CUDA and generates simulated BF16 KV cache data for 32 layers. The benchmark reports PUT/GET throughput, compression ratio, and cache statistics. `Verification: passed` indicates successful data verification. Pass `--data-file PATH` to use data from a file.

Enable Intel DSA acceleration for KV cache transfers:

```bash
./benchmark/kvstore/kvstore_benchmark.sh --dsa
```

Collect a CUDA/NVTX profile with Nsight Systems:

```bash
./benchmark/kvstore/kvstore_benchmark.sh --nsys
```

Profiles are written to `_data/nsys_report*`. The `--dsa` and `--nsys` options can be used together.
